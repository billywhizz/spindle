...
function genFunc (name, params, rtype, cif, address, dv, size) {
  ...
  //console.log(`${name}\n${source.join('\n').trim()}`)
  return spin.compile(source.join('\n').trim(), `func.js`, ['cif', 'address', 'rdv', 'getAddress', 'ffi'], [])(cif, address, rdv, spin.getAddress, ffi)
}
...
function wrap (name, address, rtype, params) {
  ...
  const cif = dv.buffer
  const status = ffi.prepare(cif, FFIArgtypeFromType(translateType(rtype)), params.map(FFIArgtypeFromType))
  if (status !== FFI_OK) throw new Error(`Bad Status ${status}`)
  const size = paramsSize(params)
  if (size === 0 && !isReturnedAsBigInt(rtype)) return () => ffi.call(cif, address)
