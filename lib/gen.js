...
void ${name}Slow(const FunctionCallbackInfo<Value> &args) {\n`
    ...
    if (result === 'void') {
      ...
      src += `  ${getType(result)} rc = ${n}(${parameters.map((p, i) => `v${i}`).join(', ')});\n`
    }
...
function headerFile (deps = [], v8flags = '--stack-trace-limit=10 --use-strict --turbo-fast-api-calls --single-threaded') {
