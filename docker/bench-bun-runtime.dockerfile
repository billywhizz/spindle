...
RUN cp spindle-perf/spin ./spindl-full
RUN make -C spindle-perf minimal
RUN cp spindle-perf/spin ./spindl-minimal
RUN make -C spindle-perf minimal-static
RUN cp spindle-perf/spin ./spindl-minimal-static
RUN curl https://bun.sh/install | bash
RUN cp /root/.bun/bin/bun ./
RUN cp spin-perf/bench/version.js ./
